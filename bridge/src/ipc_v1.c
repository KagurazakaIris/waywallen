...
int ww_req_hello_encode(const ww_req_hello_t *m, ww_buf_t *out) {
    int rc;
    (void)m;
    if ((rc = w_string(out, m->client))) return rc;
    if ((rc = w_u32(out, m->version))) return rc;
    return WW_OK;
}
...
int ww_req_hello_decode(const uint8_t *buf, size_t len, ww_req_hello_t *out) {
    ...
    ww_rd_t r = { buf, 0, len };
    int rc;
    if ((rc = rd_string(&r, &out->client))) goto fail;
    if ((rc = rd_u32(&r, &out->version))) goto fail;
    if (r.pos != r.len) {
...
int ww_req_load_scene_encode(const ww_req_load_scene_t *m, ww_buf_t *out) {
    int rc;
    (void)m;
    if ((rc = w_string(out, m->pkg))) return rc;
    if ((rc = w_string(out, m->assets))) return rc;
...
int ww_req_load_scene_decode(const uint8_t *buf, size_t len, ww_req_load_scene_t *out) {
    ...
    ww_rd_t r = { buf, 0, len };
    int rc;
    if ((rc = rd_string(&r, &out->pkg))) goto fail;
    if ((rc = rd_string(&r, &out->assets))) goto fail;
...
void ww_req_load_scene_free(ww_req_load_scene_t *m) {
    free(m->pkg); m->pkg = NULL;
    free(m->assets); m->assets = NULL;
}
...
int ww_evt_bind_buffers_encode(const ww_evt_bind_buffers_t *m, ww_buf_t *out) {
    ...
    if ((rc = w_u32(out, m->width))) return rc;
    if ((rc = w_u32(out, m->height))) return rc;
    if ((rc = w_u32(out, m->stride))) return rc;
    if ((rc = w_u64(out, m->modifier))) return rc;
    if ((rc = w_u64(out, m->plane_offset))) return rc;
    if ((rc = w_array_u64(out, &m->sizes))) return rc;
    return WW_OK;
}
...
int ww_evt_bind_buffers_decode(const uint8_t *buf, size_t len, ww_evt_bind_buffers_t *out) {
    ...
    if ((rc = rd_u32(&r, &out->width))) goto fail;
    if ((rc = rd_u32(&r, &out->height))) goto fail;
    if ((rc = rd_u32(&r, &out->stride))) goto fail;
    if ((rc = rd_u64(&r, &out->modifier))) goto fail;
    if ((rc = rd_u64(&r, &out->plane_offset))) goto fail;
    if ((rc = rd_array_u64(&r, &out->sizes))) goto fail;
    if (r.pos != r.len) {
        int rc2 = WW_ERR_TRAILING;
        (void)rc2;
...
void ww_evt_bind_buffers_free(ww_evt_bind_buffers_t *m) {
    free(m->sizes.data); m->sizes.data = NULL; m->sizes.count = 0;
}

uint32_t ww_evt_bind_buffers_expected_fds(const ww_evt_bind_buffers_t *m) {
    return (uint32_t)((uint64_t)m->count);
}
